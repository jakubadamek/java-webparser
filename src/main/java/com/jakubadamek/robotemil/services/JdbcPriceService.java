...
public class JdbcPriceService implements PriceService {
	...
	int rows = jdbcTemplate.query("SELECT * FROM Prices WHERE Date=? AND Web=? AND DaysBefore=?", rowMapper, date,
		web, daysBefore(date, new Date())).size();
	return rows;
    }
	...
	try {
	    ...
	    if (e.getCause() == null || !e.getCause().getMessage().contains("Table already exists")) {
		throw e;
	    }
	}
    }
	...
	for (String hotel : prices.getData().keySet()) {
	    ...
	    rows.add(new Object[] { web, hotel, new Date(), daysBefore(date, new Date()), date,
		    priceAndOrder.price, priceAndOrder.order });
	}
	jdbcTemplate.batchUpdate("INSERT INTO Prices(" + PRICES_COLUMNS + ") VALUES(?, ?, ?, ?, ?, ?, ?)", rows);
    }

    private void deleteRefreshedData(String web, Date date) {
	int deleted = jdbcTemplate.update("DELETE FROM Prices WHERE Date=? AND Web=? AND DaysBefore=?", date, web,
